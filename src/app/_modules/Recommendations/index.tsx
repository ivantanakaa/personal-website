...
export default function Recommendations() {
  const renderRecommendations = () => {
    return recommendations.map(({ name, title, description }, index) => {
      return (
        <li className="list-none flex items-stretch " key={`honor-${index}`}>
          <div className="mb-4 mr-4 min-w-[90vw] lg:min-w-[70vw] flex bg-white flex-col items-center justify-center shadow-xl rounded-lg p-8 ">
            <span className="text-xl font-semibold italic text-center">
              {name}
            </span>
