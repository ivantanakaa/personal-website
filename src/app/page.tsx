...
export default function ResumePage() {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Ivan_Tanaka_Resume_${currentYear}`,
    // This style block fixes the "Ugly" PDF issue by forcing
    // high-contrast colors and removing browser headers/footers
    pageStyle: `
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 font-sans selection:bg-amber-500/30">
      ...
      <div ref={componentRef} className="print:bg-white">
        ...
        <section className="max-w-6xl mx-auto px-6 py-24 print:py-10">
          ...
          <div className="space-y-16 print:space-y-8">
            {careers.map((exp, idx) => (
              ...
              >
                {/* THE VERTICAL LINE: Turns Orange on Hover */}
                ...
                {/* THE BULLET POINT: Glows and turns Orange on Hover */}
                ...
                <div className="flex flex-wrap justify-between items-baseline mb-4 gap-4">
                  <h3 className="text-2xl font-bold text-white group-hover:text-amber-400 group-hover:shadow-lg transition-colors duration-300 print:text-black print:text-lg">
                    {exp.position}
                  </h3>

                  {/* THE DATE: Now highlights on hover */}
                ...
                <ul className="grid gap-3">
                  {exp.jobs.map((job, jIdx) => (
                    <li
                      key={jIdx}
                      className="text-slate-400 text-sm leading-relaxed flex gap-3 print:text-slate-700 print:text-xs"
                    >
                      <span className="text-slate-700 group-hover:text-amber-600 mt-1.5 font-bold print:text-amber-600 transition-colors duration-300">
                        •
                      </span>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {portfolios.map((project, idx) => {
                ...
                return (
                  <a
                    key={idx}
                    href={projectUrl || "#"} // Use the string URL
                    target={hasLink ? "_blank" : "_self"}
                    rel="noopener noreferrer"
                    onClick={(e) => !hasLink && e.preventDefault()} // Disable click if no link
                ...
                hasLink
                  ...
                  >
                    <div className="bg-slate-950 p-6 rounded-lg h-full flex flex-col relative overflow-hidden">
                      {/* Only show the icon if there is a valid link */}
                      {hasLink && (
                      ...
                      <p
                        className="text-slate-500 text-xs mb-6 line-clamp-3 leading-relaxed group-hover:text-slate-300 transition-colors duration-300"
                        dangerouslySetInnerHTML={{
                      ...
                      <div className="flex flex-wrap gap-2 mt-auto">
                        {project.tags.map((tag) => (
                          <span
                            key={tag}
                            className="text-[9px] font-bold text-slate-400 border border-slate-800 px-2 py-0.5 rounded uppercase group-hover:border-amber-900 group-hover:text-amber-500/80 transition-colors duration-300"
                          >
                            {tag}
                          </span>
                        ))}
        ...
        {/* EDUCATION & HONORS */}
        <section className="max-w-6xl mx-auto px-6 py-24 grid md:grid-cols-2 gap-16 print:py-8 print:grid-cols-1 print:gap-8">
          <div className="break-inside-avoid">
            <div className="flex items-center gap-2 mb-8 text-amber-500/80 print:mb-4">
              <GraduationCap size={24} className="print:text-amber-600" />
            ...
            {educations.map((edu, idx) => (
              <div key={idx}>
                ...
                <p className="text-amber-600 mb-4 print:text-amber-700 print:text-xs">
                ...
                <ul className="space-y-2">
                  {edu.achievements.map((ach, i) => (
                    <li
                      key={i}
                      className="text-sm text-slate-400 flex gap-2 print:text-slate-700 print:text-xs"
                    >
                      <span className="text-amber-900 print:text-amber-600">
                        ▹
                      </span>
          ...
          <div className="break-inside-avoid">
            <div className="flex items-center gap-2 mb-8 text-amber-500/80 print:mb-4">
              <Award size={24} className="print:text-amber-600" />
              <h2 className="text-xl font-bold text-white print:text-black">
                Honors & Mentoring
              </h2>
            </div>
            <ul className="space-y-4">
              {honors.map((honor, idx) => (
                <li
                  key={idx}
                  className="p-4 bg-slate-900/50 rounded-lg border-l-4 border-amber-600 text-slate-300 text-sm print:bg-slate-50 print:text-slate-800 print:text-xs print:p-2"
        ...
        {/* CERTIFICATIONS */}
        <section className="py-24 border-t border-slate-900 bg-slate-950 print:bg-white print:py-8 print:border-slate-200">
          <div className="max-w-6xl mx-auto px-6">
            <h2 className="text-sm font-black text-amber-500/80 uppercase tracking-[0.3em] mb-12 print:mb-4 accent-text">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 print:grid-cols-3">
              {certificates.map((cert, idx) => (
                <a
                  key={idx}
                  href={cert.link} // The missing link is now here
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group p-4 md:p-6 bg-slate-900/0 border border-slate-800 rounded-2xl transition-all duration-300 hover:border-amber-500/50 hover:bg-amber-500/[0.02] hover:-translate-y-1 print:border-slate-200 print:p-2"
                >
                  <span className="text-[10px] font-black uppercase text-amber-400 bg-amber-400/10 px-2 py-1 rounded transition-colors group-hover:bg-amber-400 group-hover:text-slate-950 print:text-amber-600 print:bg-amber-50">
                    {cert.issued_by}
                  </span>

                  <h3 className="text-sm font-bold text-slate-200 mt-2 md:mt-4 group-hover:text-white transition-colors print:text-black print:mt-1 print:text-[11px]">
                    {cert.title}
                  </h3>
                  ...
                  <div className="flex justify-between items-center mt-2 md:mt-6">
                    ...
                    <ExternalLink
                      size={12}
                      className="text-amber-500 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0 print:hidden"
                    />
                  </div>
                </a>
              ))}
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {contacts.map((contact, idx) => (
                <a
                  ...
                  href={contact.link}
                  target="_blank"
                  className="flex items-center gap-3 px-6 py-3 bg-slate-900 border border-slate-800 rounded-full text-sm font-bold text-slate-300 hover:bg-amber-600 hover:text-white hover:border-amber-400 duration-300 transition-all active:scale-95"
                >
      ...
      <footer className="py-12 px-6 border-t border-slate-900 bg-black print:hidden">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center opacity-40">
          <p className="text-xs font-mono tracking-tighter text-white">
